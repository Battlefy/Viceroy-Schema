...
SchemaRule.isRule = function(rule) {
  ...
  for(var i = 0; i < keys.length; i += 1) {
    if(keys[i] != 'type' && keys[i] != 'match') { return false; }
  }
  ...
  if(typeof rule.match == 'object') {
    ...
    var matchCondProps = ['not', 'exists', 'lt', 'gt', 'min', 'max', 'length', 'in', 'notIn'];
    if(keys.length > matchCondProps.length) { return false; }
    for(var i = 0; i < keys.length; i += 1) {
      if(matchCondProps.indexOf(keys[i]) == -1) { return false; }
    }
  }
  return true;
}
