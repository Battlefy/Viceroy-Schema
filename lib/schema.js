  ...
  (function rec(path, rule, val, cb) {
    ...
    // preform validation on a objects and arrays
      ...
      // objects
        ...
        // create an array with all of the rule
        // properties.
        var rProps = Object.keys(rule);

        // If this schema is exclusive and any
        // extra properties are found, then
        // callback with an error.
        if(_this.opts.exclusive) {
          for(var prop in val) {
        ...
        // If this schema is inclusive and any
        // rule properties are not found, then
        // callback with an error.
        if(_this.opts.inclusive) {
          for(var i = 0; i < rProps.length; i += 1) {
            if(val[rProps[i]] === undefined) {
              errors = errors || {};
              errors[rProps[i]] = new Error(path.concat(prop).join('.') + ' must exist');
            }
          }
          if(errors) { return cb(errors); }
        }
...
Schema.prototype._createRules = function(rules) {
  ...
  return (function rec(rules) {
    var schema;
    if(typeof rules.length == 'number') { schema = []; }
    else { schema = {}; }
...
Schema.prototype._extractOpts = function(rules) {
  var opts = {};
  if(rules.$inclusive) {
    if(typeof rules.$inclusive != 'boolean') { throw new Error('rules.$inclusive must be a boolean'); }
    opts.inclusive = rules.$inclusive;
    delete rules.$inclusive;
  }
  if(rules.$exclusive) {
    if(typeof rules.$exclusive != 'boolean') { throw new Error('rules.$exclusive must be a boolean'); }
